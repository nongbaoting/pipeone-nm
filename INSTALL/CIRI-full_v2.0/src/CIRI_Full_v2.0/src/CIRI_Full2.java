	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		if(!z.get(0).equals("-h")){
			...
			if(breakflag==0){
				...
				Merge3 merge=new Merge3(workdir+"/CIRI_output/"+prefix+".ciri",workdir+"/CIRI-AS_output/"+prefix+"_jav.list",workdir+"/CIRI-full_output/"+prefix+"_ro2_info.list",anno,ref,workdir+"/CIRI-full_output/"+prefix);
				...
				if(z.indexOf("-c")!=-1){
					...
					System.out.println("CIRI output file not set, exit");
					breakflag=1;
				}
				...
				if(z.indexOf("-as")!=-1){
					...
					System.out.println("ciri-as output file (prefix_jav.list) not set, Exit");
					breakflag=1;
				}
				if(z.indexOf("-ro")!=-1){
					...
					System.out.println("ciri_ro file not set. Use ciri-as result only");
				}
				if(breakflag==0){
					Merge3 merge=new Merge3(ciri,ciri_as,ciri_ro,gtf,ref,out);
