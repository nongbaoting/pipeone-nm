...
class MYRUN:
    ...
    def combine_coding_prediction(self, tx_gene,  CPAT_res, PLEK_res, CPPred_res,
        ...
        with open("coding_potential_sum.tsv", 'w') as fo, open(noncoding_ls, 'w') as fl:
            ...
            for tx_id in txs:
                fo.write(
                    '\t'.join([tx_id, d_cpat[tx_id], d_plek[tx_id], d_cppred[tx_id]]) + '\n')
    ...
    def gffcompare_tmap(self, tmap, out, single_exon=0):
        ...
        with open(tmap) as f, open(out, 'w') as fo:
            ...
            for li in f:
                ...
                mygene_exon[qry_gene_id] = max(
                    mygene_exon[qry_gene_id], num_exons)

                mytx[qry_id] = [qry_gene_id, class_code,
                                int(num_exons), int(tx_len)]
