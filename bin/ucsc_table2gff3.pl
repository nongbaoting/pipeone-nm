...
#		$data{exonFrames}  = undef; # not present in this table
		...
		$data{sp}        = $kgxref->{ $linedata[0] }->[1] || undef; # SwissProt ID
		$data{spd}       = $kgxref->{ $linedata[0] }->[2] || undef; # SwissProt display ID
...
#	my $current_chrom;
	...
	my %2count; # hash to a in generating unique primary IDs
	my %counts = (
		'gene'       => 0,
		'mrna'       => 0,
		'pseudogene' => 0,
	...
	while (my $line = $table_fh->getline) {
		...
		my $transcript = generate_new_transcript($linedata, \%2count);
